...
from services import create_user_normal_auth,get_user_by_id,get_user_by_google_id
...
def create_user(details:RegisterDetails,db:Session=Depends(get_db)):
    ...
    google_user=get_user_by_google_id(db=db,email=details.email)
    print(user,google_user)
    if user or google_user:
...
def login_for_access_token(
    ...
    user=get_user_by_id(db=db,email=user.email)
    ...
    response.set_cookie(
        ...
        samesite="lax",           # "none" if cross-origin + secure=True
        path="/",
